...
/**
 * EVM v2 SMOD operation using long[] stack representation.
 *
 * <p>Performs signed 256-bit modulo reduction using two's complement representation. The sign of
 * the result follows the sign of the dividend. If the modulus is zero, the result is zero per EVM
 * specification.
 */
public class SModOperationV2 extends AbstractFixedCostOperationV2 {

  private static final Operation.OperationResult SMOD_SUCCESS =
  ...
  /**
   * Performs SMOD on the v2 long[] stack.
   *
  ...
  public static Operation.OperationResult staticOperation(
    ...
    if (s[b] == 0L && s[b + 1] == 0L && s[b + 2] == 0L && s[b + 3] == 0L) {
      ...
      final UInt256 result = val0.signedMod(val1);
