...
- Add `--slow-block-threshold=<MILLISECONDS>` CLI option for slow-block execution metrics: `-1` disables (default), `0` logs every block, and a positive value logs only blocks whose total processing time meets or exceeds it. Blocks that meet the threshold emit a JSON document on a dedicated `SlowBlock` logger with timing, throughput, state read/write, cache hit/miss and EVM operation counts following the cross-client slow-block metrics format, plus a Besu `bal` section reporting the work BAL block processing does off the import thread. Metrics are collected for blocks that carry a Block Access List, so the feature activates at the Amsterdam fork.
