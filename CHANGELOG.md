...
### Bug fixes
- Serialize `BftMiningCoordinator` lifecycle transitions (`start`/`stop`/`enable`/`disable`) with a lock so their state changes and side effects can no longer interleave when invoked concurrently from different threads. [#10887](https://github.com/besu-eth/besu/pull/10887)
