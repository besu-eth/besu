...
- DiscV5 peer discovery now throttles to a slow cadence (default 30s, tunable with `--Xv5-slow-discovery-interval-seconds`) once connected peers reach `--Xv5-minimum-peer-ratio` of `--max-peers`, instead of stopping peer search and DiscV5 outbound connection attempts entirely with no periodic retry. `--Xv5-minimum-peer-ratio` is now validated as greater than 0 and at most 1; values above 1 previously disabled the throttle and pinned discovery to a one-second cadence. [#11344](https://github.com/besu-eth/besu/pull/11344)
