...
- Add discovery/RLPx observability metrics: `besu_network_discovery_demux_packets_total{protocol}` (shared-socket packet routing), `besu_network_discv4_known_peers_current` (DiscV4 routing table size), `besu_network_rlpx_outbound_connect_attempts_total{source}` and `besu_network_rlpx_connect_outcome_total{result}` (outbound RLPx connection attempts and their outcome by originating source), and `besu_network_discv5_discovery_round_duration_seconds`/`besu_network_discv5_discovery_round_total{outcome}` (DiscV5 discovery round timing and success/timeout rate). [#PR_NUMBER](https://github.com/besu-eth/besu/pull/PR_NUMBER)
