...
- Extract the Plugin API consensus module. The proof-of-authority and BFT query contracts (`PoaQueryService` and `BftQueryService`) now live in a new `besu-plugin-api-consensus` artifact, re-exported by `besu-plugin-api` so existing consumers are unaffected. [#11043](https://github.com/besu-eth/besu/pull/11043)
