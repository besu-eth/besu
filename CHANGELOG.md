...
- Fix `RLPException` when a 26.1.0+ node decodes QBFT `RoundChange` or legacy-IBFT `RoundChange`/`Proposal` messages from a pre-26.1.0 peer. Enables rolling upgrades from `25.x` to `26.x+` without consensus disruption.
