...
- A snap sync header batch that does not chain onto the cached pivot now raises `WrongChainException` and re-pivots immediately, instead of an `IllegalStateException` that fell through to stall detection. The pivot can be captured mid-reorg, so the very first batch can legitimately fail to link; recovery took up to 20 retries backing off to 30s — and logged an ERROR with a stack trace — before escalating to the same re-pivot. [#10973](https://github.com/besu-eth/besu/issues/10973)
