...
### Bug fixes
- EIP-7928's block access list item budget is now checked only against the access list built during block execution, matching the reference implementation. Checking the supplied list before execution rejected nothing extra but pre-empted the errors execution would have reported, so a block invalid for an unrelated reason was blamed on its access list.
