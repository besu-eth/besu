...
- Fix unbounded `LayeredKeyValueStorage` accumulation under a stalled forkchoice head: `engine_newPayload` now returns SYNCING when the cached world-state layer count exceeds 128, preventing the heap growth and GC pressure that previously caused OOM. Also fixes a feedback loop where the SYNCING response itself prevented `forkchoiceUpdated` from arriving (the CL stops sending FCU while the EL returns SYNCING), causing stale layers to never be evicted; layers are now evicted by count on each `engine_newPayload` call when over threshold, breaking the deadlock. ([#10509](https://github.com/besu-eth/besu/issues/10509))
