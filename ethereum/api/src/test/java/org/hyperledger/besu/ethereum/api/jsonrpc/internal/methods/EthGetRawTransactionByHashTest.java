...
public class EthGetRawTransactionByHashTest {

  // A legacy (FRONTIER) transaction RLP-encoded, used for round-trip verification.
  private static final String VALID_TRANSACTION_HEX =
  ...
  void shouldReturnRawTransactionForFrontierTransaction() {
    final Transaction transaction =
        Transaction.readFrom(Bytes.fromHexString(VALID_TRANSACTION_HEX));
    ...
    final JsonRpcRequest request =
        new JsonRpcRequest(
            JSON_RPC_VERSION,
            method.getName(),
            new Object[] {transaction.getHash().toHexString()});
    final JsonRpcRequestContext context = new JsonRpcRequestContext(request);

    // The expected raw bytes are encodeOpaqueBytes(BLOCK_BODY) for the transaction.
