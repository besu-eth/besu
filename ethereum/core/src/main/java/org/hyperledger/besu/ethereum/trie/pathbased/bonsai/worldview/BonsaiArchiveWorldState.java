...
/**
 * Bonsai world state used to serve archive state proofs. It behaves like {@link BonsaiWorldState}
 * except that when frozen it wraps its storage in a {@link BonsaiArchiveWorldStateLayerStorage} so
 * flat reads use the historical (checkpoint) block context held in the layer, rather than the live
 * HEAD context that the default {@code BonsaiWorldStateLayerStorage} would resolve.
 */
