...
public interface StateRootCommitter {
  ...
  StateRootComputation compute(MutableWorldState worldState, BlockHeader blockHeader);
  ...
  default StateRootCommitter timed(final OperationTimer timer) {
    ...
    return (worldState, blockHeader) -> {
      try (var ignored = timer.startTimer()) {
        return delegate.compute(worldState, blockHeader);
      }
    };
  }
}
