...
class MainnetBlockAccessListValidatorTest {
  ...
  @Nested
  class Eip7928ItemCountHelpers {

    @Test
    void eip7928ItemCountMatchesDefinition() {
      final BlockAccessList.AccountChanges account =
          new BlockAccessList.AccountChanges(
              ADDR_1,
              List.of(
              ...
              List.of());
      final BlockAccessList bal = new BlockAccessList(List.of(account));
      Assertions.assertThat(bal.eip7928ItemCount()).isEqualTo(3L);
    }

    @Test
    void validateExecutedBlockAccessListItemSizeUsesForkGasCalculator() {
      final ProtocolSchedule protocolSchedule = mock(ProtocolSchedule.class);
      final ProtocolSpec protocolSpec = mock(ProtocolSpec.class);
      final org.hyperledger.besu.evm.gascalculator.GasCalculator gasCalculator =
          mock(org.hyperledger.besu.evm.gascalculator.GasCalculator.class);
      when(gasCalculator.getBlockAccessListItemCost()).thenReturn(2000L);
      when(protocolSpec.getGasCalculator()).thenReturn(gasCalculator);
      when(protocolSchedule.getByBlockHeader(any())).thenReturn(protocolSpec);
      final MainnetBlockAccessListValidator validator =
          new MainnetBlockAccessListValidator(protocolSchedule);
      final BlockHeader header = new BlockHeaderTestFixture().gasLimit(6000L).buildHeader();
      Assertions.assertThat(validator.validateExecutedBlockAccessListItemSize(3, header)).isEmpty();
      Assertions.assertThat(validator.validateExecutedBlockAccessListItemSize(4, header))
          .isPresent();
    }
  }

  @Nested
  class ExecutedBlockAccessListAfterBuild {

    @Test
    void afterBuildSucceedsWhenBalHashMatchesAndWithinItemBudget() {
      final BlockAccessList.AccountChanges a1 =
          new BlockAccessList.AccountChanges(
              ...
              List.of(),
              List.of());
      final BlockAccessList executedBal = new BlockAccessList(List.of(a1));
      final BlockHeader header = headerWithBal(executedBal, 6000L);
      Assertions.assertThat(
              validator()
                  .validateExecutedBlockAccessListAfterBuild(
                      executedBal, header, Optional.empty(), false))
          .isEmpty();
    }

    @Test
    void afterBuildReturnsErrorWhenExecutedBalHashDoesNotMatchHeader() {
      final BlockAccessList executedBal = new BlockAccessList(List.of());
      final BlockHeader header =
      ...
      Assertions.assertThat(
          ...
          .isPresent()
          .get()
          .extracting(BlockAccessListValidationError::errorMessage)
          .asString()
          .contains("hash mismatch");
    }

    @Test
    void afterBuildReturnsSizeErrorBeforeHashWhenItemBudgetExceededEvenIfHashMatches() {
      final BlockAccessList.AccountChanges a1 =
          new BlockAccessList.AccountChanges(
              ADDR_1,
              List.of(
      ...
      final BlockAccessList.AccountChanges a2 =
          new BlockAccessList.AccountChanges(
              ...
              List.of(),
              List.of());
      final BlockAccessList executedBal = new BlockAccessList(List.of(a1, a2));
      final BlockHeader header =
          new BlockHeaderTestFixture()
              .gasLimit(6000L)
              .balHash(BodyValidation.balHash(executedBal))
              .buildHeader();
      Assertions.assertThat(
              validator()
                  .validateExecutedBlockAccessListAfterBuild(
          ...
          .extracting(BlockAccessListValidationError::errorMessage)
          .asString()
          .contains("size exceeds maximum");
    }

    @Test
    void afterBuildReturnsEmptyWhenHeaderOmitsBalHashEvenIfItemBudgetWouldFail() {
      final BlockAccessList.AccountChanges a1 =
          new BlockAccessList.AccountChanges(
              ADDR_1,
              List.of(
              ...
              List.of());
      final BlockAccessList executedBal = new BlockAccessList(List.of(a1));
      final BlockHeader header = new BlockHeaderTestFixture().gasLimit(10_000L).buildHeader();
      Assertions.assertThat(header.getBalHash()).isEmpty();
      Assertions.assertThat(
              validator()
                  .validateExecutedBlockAccessListAfterBuild(
